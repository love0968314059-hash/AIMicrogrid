...
- **Three.js驱动** - 实时3D渲染
- **交互控制** - 鼠标旋转/缩放
- **点击交互** - 点击3D场景中的设备查看详细信息
- **实时数据显示** - 功率、SOC、电价等
- **动态效果** - 风机旋转、电力流动粒子
- **控制面板** - 模拟控制和设备操作
- **策略管理** - 策略执行情况监控和策略对比分析
- **智能聊天** - 集成NLP交互
- **长期模拟** - 支持一个月（30天）的连续模拟运行
...
- **视角控制** - 鼠标拖动旋转，滚轮缩放
- **点击交互** - 点击3D场景中的任何设备（光伏、风电、电池、负荷、电网）查看详细信息
- **状态监控** - 实时显示各组件功率和SOC
- **控制面板** - 电池充放电、柴油机开关
- **策略管理面板** - 
  - **执行情况**：查看当前策略的执行状态，包括RL置信度、探索率、训练步数等
  - **策略对比**：对比RL策略、规则策略和随机策略的性能
- **功率图表** - 历史趋势可视化（显示最近24小时数据）
- **智能聊天** - 输入问题获取系统信息
- **长期模拟** - 支持一个月（30天，43200步）的连续模拟，显示进度和剩余天数
